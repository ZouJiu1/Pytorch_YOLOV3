...
def trainer():
    ...
    NCCL_DEBUG=WARN
    python -m torch.distributed.launch --nproc_per_node=3 --nnodes=1 --node_rank=0 --master_addr="localhost" --master_port=666 allcodes/train_yolovkkn_seg_distribute.py
    python -m torch.distributed.launch --nproc_per_node=6 --nnodes=1 --node_rank=0 --master_addr="localhost" --master_port=666 allcodes/train_yolovkkn_seg_distribute.py
    python -m torch.distributed.launch --nproc_per_node=2 --nnodes=1 --node_rank=0 --master_addr="localhost" --master_port=666 allcodes/train_yolovkkn_seg_distribute.py
    python -m torch.distributed.launch --nproc_per_node=1 --nnodes=1 --node_rank=0 --master_addr="localhost" --master_port=666 allcodes/train_yolovkkn_seg_distribute.py
    e: \d\d, r:99.
    '''
    ...
    dataloader = DataLoader(traindata, batch_size = batch_size, shuffle = (train_sampler is None), \
        num_workers=num_cpu, collate_fn=collate_fnseg, pin_memory=True, sampler=train_sampler)
    ...
    if rank==0:
        ...
        valdataloader = DataLoader(valdata, batch_size = batch_size, shuffle=False, \
                    num_workers=num_cpu, collate_fn=collate_fnseg, pin_memory=True) #, sampler=val_sampler)
...
# # Confi loss: {:.3f}, iouloss: {:.3f}, Loss: {:.3f}, avgloss: {:.3f}'''
    ...
    for epoch in range(nowepoch, num_epochs + 1):
        if rank == 0:
            print(('\n' + '%9s' * (6 + 6 + 3)) % ('Epoch', 'ratio', 'iter', 'alliter', 'lr', 'Class_L', 'Confi_L', "iouL", "Loss", "avgl", \
                "iounow", "Confi", "NO_Confi", "Class", "Mask"))
        # if rank == 0:
                ...
                try:
                    ...
                    if chooseLoss == "darknetRevise":
                        ...
                        loss, c_l, confi_l, iouloss, iounow, cof, ncof, cla, maskloss, boxnum = calculate_losses_yolofive(prediction, labels, model, ignore_thresh, iou_thresh, count_scale, \
                                                                                            bce0loss, bce1loss, bce2loss, bcecls, bcecof, mseloss, gtmask)
                        # loss, c_l, confi_l, iouloss, iounow, cof, ncof, cla, maskloss, boxnum = calculate_losses_yolofive_revise(prediction, labels, model, ignore_thresh, iou_thresh, count_scale, \
                        #                                                                     bce0loss, bce1loss, bce2loss, bcecls, bcecof, mseloss, gtmask)
...
Confi: {:.3f}, iou: {:.3f}, Loss: {:.3f}, avgloss: {:.3f}, iounow: {:.3f}, cof: {:.3f}, ncof: {:.6f}, cla: {:.3f}, mask: {:.3f}'''.format(
            ...
            if rank == 0 and i%1==0:
                # print(logword)
                pbar.set_description(('%9s'+'%9.5g'+"%9s"+"%9s"+ '%9.5g' * 11) %
                                     (epoch, i*100/length, i+1, stepiters, optimizer.state_dict()['param_groups'][0]['lr'], \
                                    float(c_l.item()/boxnum), float(confi_l.item()/boxnum), iouloss.item()/boxnum, loss/boxnum, \
                                        epoch_loss/boxnum, iounow.item(), cof.item(), ncof.item(), cla.item(), maskloss.item()))
                flogs.write(logword+'\n')
        ...
        if rank == 0 and (epoch in jkk or epoch==num_epochs):
            ...
            [map, mAP0], lengthkk = validation_map_seg(model if ema==None else ema.ema, yolovfive, valdataloader, devicenow) #, score_thresh, nms_thresh)
