...
from models.Yolofastestrevise import yolofastestNet, computeloss
import torch.optim as optim
from utils.utils_730 import load_darknet_weights, intialization, freeze_darknet_backbone
from utils.common import cvshow, collate_fn, provide_determinism
from utils.validation_yolofastestrevise import validation_map
from torch.utils.data import Dataset, DataLoader
from loaddata.load_datas_yolofastestrevise import trainDataset
...
def trainer():
    ...
    dataloader = DataLoader(traindata, batch_size=batch_size,shuffle=True, \
        num_workers=cpu_count()//3, collate_fn=collate_fn)
    ...
    for epoch in range(num_epochs):
        ...
        for i, (image, labels) in enumerate(dataloader):
            ...
            logword = '''epoch: {}, ratio:{:.2f}%, iteration: {}, alliters: {}, lr: {:.6f}, IoU loss: {:.3f}, Object loss: {:.3f}, \
Class loss: {:.3f}, Loss: {:.3f}, Batch loss: {:.3f}, avgloss: {:.3f}'''.format(
                       epoch, i*100/length, i+1, stepiters, optimizer.state_dict()['param_groups'][0]['lr'], float(loss_components[0]),\
                        float(loss_components[1]), float(loss_components[2]), float(loss_components[3]), loss, epoch_loss)
            print(logword)
            flogs.write(logword+'\n')
            flogs.flush()
        ...
        if(pre_map < map) or (epoch+1)%1==0 or epoch==num_epochs-1:
            torch.save(savestate, __savepath__+os.sep+r'model_{}_{}_map{}_{:.3f}_{}.pth'.format(epoch, stepiters, map, loss,tim))
            print('savemodel ')
            pre_map = map
        # except:
