...
import datetime
import numpy as np
from models.Yolov3copy2 import Yolov3
import torch.optim as optim
from utils.utils_717_730 import load_darknet_weights, intialization, freeze_darknet_backbone
from utils.validation import validation_map, validation_map_myself
from utils.common import cvshow, validDataset, collate_fn
from torch.utils.data import Dataset, DataLoader
from loaddata.load_datas_717_730 import trainDataset
...
def trainer():
    ...
    model = Yolov3(num_classes, anchors, strides, ignore_thresh, \
        inputwidth,device, score_thresh = score_thresh, nms_thresh = nms_thresh)
    print(model)
    ...
    validloader = DataLoader(validdata, batch_size=1,shuffle=True, num_workers=8)
    start = time.time()
...
recall50: {:.3f}, recall75: {:.3f}, loss: {:.3f}, avgloss: {:.3f}, recall: {:.3f}, Precision: {:.3f}, classes: {:.3f}'''.format(
        ...
        map = validation_map_myself(model, validloader)
        model.train()
        print("validation......num_img: {}, mAP: {}, premap:{}".format(len(validdata), map, pre_map))
        try:
            if(pre_map < map) or epoch%3==2:
                torch.save(savestate, r'/home/savemodel/model_{}_{}_map{}_{:.3f}_{}.pth'.format(epoch, stepiters, map, loss.item(),tim))
            print('savemodel ')
        except:
