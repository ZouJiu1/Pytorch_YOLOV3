...
from models.layer_loss_segment_keypoint import calculate_losses_darknet, calculate_losses_Alexeydarknet, calculate_losses_yolofive, \
...
from utils.common import cvshow_seg_kpt, collate_fnseg_keypoint, provide_determinism, smart_optimizer, ModelEMA, de_parallel
from utils.validation_yolov3tiny import validation_map_segkeypoint
from torch.utils.data import Dataset, DataLoader
from loaddata.cocoread_seg_keypoint import trainDataset
...
def trainer():
    ...
    NCCL_DEBUG=WARN
    python -m torch.distributed.launch --nproc_per_node=3 --nnodes=1 --node_rank=0 --master_addr="localhost" --master_port=666 allcodes/train_yolovkkn_seg_keypoint_distribute.py
    python -m torch.distributed.launch --nproc_per_node=6 --nnodes=1 --node_rank=0 --master_addr="localhost" --master_port=666 allcodes/train_yolovkkn_seg_keypoint_distribute.py
    python -m torch.distributed.launch --nproc_per_node=2 --nnodes=1 --node_rank=0 --master_addr="localhost" --master_port=666 allcodes/train_yolovkkn_seg_keypoint_distribute.py
    python -m torch.distributed.launch --nproc_per_node=1 --nnodes=1 --node_rank=0 --master_addr="localhost" --master_port=666 allcodes/train_yolovkkn_seg_keypoint_distribute.py
    e: \d\d, r:99.
    '''
    ...
    dataloader = DataLoader(traindata, batch_size = batch_size, shuffle = (train_sampler is None), \
        num_workers=num_cpu, collate_fn=collate_fnseg_keypoint, pin_memory=True, sampler=train_sampler)
    ...
    if rank==0:
        ...
        valdataloader = DataLoader(valdata, batch_size = batch_size, shuffle=False, \
                    num_workers=num_cpu, collate_fn=collate_fnseg_keypoint, pin_memory=True) #, sampler=val_sampler)

    if rank == 0:
        flogs = open(logfile, 'w')
    model = YolovKKNet(0, anchors, devicenow, inputwidth)
    ...
    #     notfreeze_netlist = ["cv60", "cv61", "cv62", "p", "out0", "out1", "out2"]
    #     for name, m in model.named_parameters(): #1 
    #         mar = 999
    #         for i in notfreeze_netlist:
    #             if i in name:
    #                 mar = -999
    #                 break
    #         if mar < 0:
    ...
    #     optimizer = optim.Adam(params, lr=learning_rate, betas=(momnetum, 0.999), weight_decay= weight_decay)  # adjust beta1 to momentum
    # else:
    # optimizer = optim.SGD([i for i in model.parameters() if i.requires_grad_], lr=learning_rate, momentum=momnetum, nesterov=True, weight_decay= weight_decay)
    ...
    bce2loss = torch.nn.BCEWithLogitsLoss(reduction='none').to(devicenow)
...
# # Confi loss: {:.3f}, iouloss: {:.3f}, Loss: {:.3f}, avgloss: {:.3f}'''
    ...
    for epoch in range(1, num_epochs + 1):
        if rank == 0:
            print(('\n' + '%9s' * (6 + 7 + 2)) % ('Epoch', 'ratio', 'iter', 'alliter', 'lr', 'confi_kpt', 'Confi_L', "iouL", "Mask", "KPT", "Loss", "avgl", \
                "iounow", "Confi", "NO_Confi"))
        # if rank == 0:
            ...
            with torch.cuda.amp.autocast(amp):
                ...
                try:
                    ...
                    if chooseLoss == "darknetRevise":
                        loss, confi_kpt, confi_l, iouloss, iounow, cof, ncof, maskloss, kptloss, boxnum = calculate_losses_darknetRevise(prediction, labels, model, ignore_thresh, \
                                                                                            bce0loss, bce1loss, bce2loss, bcecls, bcecof, mseloss, gtmask, keypoints)
                    elif chooseLoss == "darknet":
                        loss, confi_kpt, confi_l, iouloss, iounow, cof, ncof, maskloss, kptloss, boxnum = calculate_losses_darknet(prediction, labels, model, ignore_thresh, \
                                                                                            bce0loss, bce1loss, bce2loss, bcecls, bcecof, mseloss, gtmask, keypoints)
                    elif chooseLoss == "Alexeydarknet":
                        loss, confi_kpt, confi_l, iouloss, iounow, cof, ncof, maskloss, kptloss, boxnum = calculate_losses_Alexeydarknet(prediction, labels, model, ignore_thresh, iou_thresh, count_scale, \
                                                                                            bce0loss, bce1loss, bce2loss, bcecls, bcecof, mseloss, gtmask, keypoints)
                    elif chooseLoss == "yolofive":
                        loss, confi_kpt, confi_l, iouloss, iounow, cof, ncof, maskloss, kptloss, boxnum = calculate_losses_yolofive(prediction, labels, model, ignore_thresh, iou_thresh, count_scale, \
                                                                                            bce0loss, bce1loss, bce2loss, bcecls, bcecof, mseloss, gtmask, keypoints)
                        # loss, c_l, confi_l, iouloss, iounow, cof, ncof, cla, maskloss, kptloss, boxnum = calculate_losses_yolofive_revise(prediction, labels, model, ignore_thresh, iou_thresh, count_scale, \
                        #                                                                     bce0loss, bce1loss, bce2loss, bcecls, bcecof, mseloss, gtmask, keypoints)
                    elif chooseLoss == "20230730":
                        loss, confi_kpt, confi_l, iouloss, iounow, cof, ncof, maskloss, kptloss, boxnum = calculate_losses_20230730(prediction, labels, model, count_scale, ignore_thresh, \
                                                                                        bce0loss, bce1loss, bce2loss, bcecls, bcecof, mseloss, gtmask, keypoints)
                    else:
                        exit(-1)
...
Confi: {:.3f}, iou: {:.3f}, Loss: {:.3f}, avgloss: {:.3f}, iounow: {:.3f}, cof: {:.3f}, ncof: {:.6f}, mask: {:.3f}, kpt: {:.3f}'''.format(
                                   epoch, i*100/length, i+1, stepiters, optimizer.state_dict()['param_groups'][0]['lr'], \
                                    float(confi_kpt.item()/boxnum), float(confi_l.item()/boxnum), iouloss.item()/boxnum, loss/boxnum, \
                                        epoch_loss/boxnum, iounow.item(), cof.item(), ncof.item(), maskloss.item(), kptloss.item())
            # if i%subsiz==0 or i == len(dataloader) - 1:
            if rank == 0 and i%1==0:
                # print(logword)
                pbar.set_description(('%9s'+'%9.5g'+"%9s"+"%9s"+ '%9.5g' * (10+1)) %
                                     (epoch, i*100/length, i+1, stepiters, optimizer.state_dict()['param_groups'][0]['lr'], \
                                    float(confi_kpt.item()/boxnum), float(confi_l.item()/boxnum), iouloss.item()/boxnum, maskloss.item()/boxnum, kptloss.item()/boxnum, loss/boxnum, \
                                        epoch_loss/boxnum, iounow.item(), cof.item(), ncof.item()))
                flogs.write(logword+'\n')
        # if rank==0 and ema:
        ...
        if rank == 0 and (epoch in jkk or epoch==num_epochs):
            ...
            [map, mAP0], lengthkk = validation_map_segkeypoint(model if ema==None else ema.ema, yolovfive, valdataloader, devicenow) #, score_thresh, nms_thresh)
