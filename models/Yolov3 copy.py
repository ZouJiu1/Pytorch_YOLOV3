...
class yolo(nn.Module):
    ...
    def __init__(self, num_classes, anchors, strides, ignore_thresh, mask,device,\
        ...
        self.anchors = self.anchors/self.strides[:, np.newaxis]             #[9, 2]
    ...
    def forward(self, prediction, gt):
        ...
        noobjmask = self.BoolTensor(batch_size, num_anchors, width, height).fill_(0)
        ...
        # print(mapsize, self.gt.size(), mapsize[[0,3,6]])
        mapsize = mapsize[[0,3,6]]
        # groundtruth = self.gt[:, np.newaxis] * mapsize[:, np.newaxis]
        # print(groundtruth.size())
        # groundtruth = torch.reshape(groundtruth, (-1, 6))
        # # print(tmpw)
        # print(groundtruth.size())
        # print(groundtruth)
        # exit(0)
        # self.gt[:, 1] = self.gt[:, 1]*width     # [label数量, 1]  center x
        # self.gt[:, 3] = self.gt[:, 3]*width     # w
        # self.gt[:, 2] = self.gt[:, 2]*height  # center y
        # self.gt[:, 4] = self.gt[:, 4]*height  # h
        for bs in range(batch_size):
             #self.gt的最后一列是归属于这个图片的label的标志
            batch_gt = self.gt[self.gt[:, -1]==bs][:, :-1] #[label数量, 5] 包括第一列类别 xywh
            batch_gt = self.FloatTensor(batch_gt).to(self.device) #xywh 

            # repeat = groundtruth[groundtruth[:, -1]==bs][:, :-1]
            batch_wh = self.FloatTensor(batch_gt[:, 3:]) #[label数量, four] 不包括类别只有坐标，只考虑wh，不考虑位置xy  wh
            anchorxyxy = torch.zeros((9, 3+1))
            anchorxyxy[:, 0] = 100 - self.anchors[:, 0] * 0.5
            anchorxyxy[:, 1] = 100 - self.anchors[:, 1] * 0.5
            anchorxyxy[:, 2] = 100 + self.anchors[:, 0] * 0.5
            anchorxyxy[:, 3] = 100 + self.anchors[:, 1] * 0.5
            # print()
            # print(batch_gtxyxy)
            # print(anchorxyxy)
            # print(self.anchors)
            batch_gtxyxy = torch.zeros((3, 4))
            allmask = []
            for aer in range(len(batch_wh)):
                boxwh = batch_wh[aer]
                boxwh = boxwh[np.newaxis, :] * mapsize[:, np.newaxis]
                batch_gtxyxy[:, 0] = 100 - boxwh[:, 0] * 0.5 #wh -> xyxy
                batch_gtxyxy[:, 1] = 100 - boxwh[:, 1] * 0.5 #wh -> xyxy
                batch_gtxyxy[:, 2] = 100 + boxwh[:, 0] * 0.5 #wh -> xyxy
                batch_gtxyxy[:, 3] = 100 + boxwh[:, 1] * 0.5 #wh -> xyxy
                ioucol = []
                for rea in range(3):
                    tgt = torch.unsqueeze(batch_gtxyxy[rea, :], 0)
                    iouanchor = box_iou(tgt, anchorxyxy[rea*3:(rea+1)*3, :])   #(n, m)
                    ioucol.extend(iouanchor[0].detach().cpu().numpy())
                ioucol = torch.from_numpy(np.array(ioucol, dtype=np.float))
                ioumask = ioucol > 0.3
                iousum = torch.sum(ioumask).item()
                if(iousum==0):   #都小于阈值的，将最大值逐一赋值给矩阵mask
                    _, iouind= torch.max(ioucol, dim=0)
                    ioumask[iouind] = True
                allmask.append(ioumask.detach().cpu().numpy())

            batch_gt[:, 1:5] = batch_gt[:, 1:5] * width
            ioumask = torch.from_numpy(np.array(allmask, dtype=np.bool))
            ioumask = ioumask[:, self.mask]
            gtxy = batch_gt[:, 1:3].long() #self.LongTensor(batch_gt[:, 1:3]) #找到对应的坐标位置 [label数量, 2]
            objmask[bs, :, gtxy[:, 0], gtxy[:, 1]] = ioumask.T

            # tx[bs, :, gtxy[:,1], gtxy[:,0]][ioumask.T] = batch_gt[:, 1] * scale
            predx = predx * scale
            predy = predy * scale
            predw = predw * scale
            predh = predh * scale
            for ik in range(3):
                tmpmask = ioumask[:, ik]
                centerx = batch_gt[:, 1] * scale
                centery = batch_gt[:, 2] * scale
                widthbox = batch_gt[:, 3] * scale
                heightbox = batch_gt[:, 4] * scale
                centerx[~tmpmask] = 0
                centery[~tmpmask] = 0
                widthbox[~tmpmask] = 0
                heightbox[~tmpmask] = 0
                clasmask = torch.zeros(classes[bs, ik, gtxy[:,1], gtxy[:,0], batch_gt[:, 0].long()].size())
                ttk = torch.zeros(targetmask[bs, ik, gtxy[:,1], gtxy[:,0], :].size(), dtype=torch.bool)
                ttk[tmpmask] = True
                clasmask[tmpmask] = 1

                tx[bs, ik, gtxy[:,1], gtxy[:,0]] = centerx
                ty[bs, ik, gtxy[:,1], gtxy[:,0]] = centery
                tw[bs, ik, gtxy[:,1], gtxy[:,0]] = widthbox
                th[bs, ik, gtxy[:,1], gtxy[:,0]] = heightbox
                classes[bs, ik, gtxy[:,1], gtxy[:,0], batch_gt[:, 0].long()] = clasmask
                targetmask[bs, ik, gtxy[:,1], gtxy[:,0], :] = ttk

                pred_x = torch.unsqueeze(predx[bs, ik, gtxy[:,1], gtxy[:,0]][tmpmask], 1)
                pred_y = torch.unsqueeze(predy[bs, ik, gtxy[:,1], gtxy[:,0]][tmpmask], 1)
                pred_w = torch.unsqueeze(predw[bs, ik, gtxy[:,1], gtxy[:,0]][tmpmask], 1)
                pred_h = torch.unsqueeze(predh[bs, ik, gtxy[:,1], gtxy[:,0]][tmpmask], 1)
                predcoord = torch.cat([pred_x, pred_y, pred_w, pred_h], dim=1)
                p_g_iou = iou_box(batch_gt[:, 1:5][tmpmask] * scale, predcoord)
                giou = GIOU_xywh_torch(batch_gt[:, 1:5][tmpmask] * scale, predcoord)
                scoremask = torch.zeros(iou_score[bs, ik, gtxy[:, 1], gtxy[:, 0]].size(), dtype=torch.float)
                scoremask[tmpmask] = giou.float()
                iou_score[bs, ik, gtxy[:, 1], gtxy[:, 0]] = scoremask
                
                # p_g_iou = (1.0 - )
                boxloss_scale = 2.0 - 1.0 * widthbox[tmpmask] * heightbox[tmpmask]/(self.inputwidth ** 2)
                if(len(p_g_iou) > 0):
                    p_g_iou = (1.0 - p_g_iou) * boxloss_scale;
                    # p_g_iou = p_g_iou * boxloss_scale;
                    # lossiou += torch.sum(p_g_iou).item()
                if(len(giou)>0):
                    giou = (1.0 - giou) * boxloss_scale
                    glossiou.append(torch.sum(giou).item())

                nomask = ~tmpmask
                if(len(p_g_iou)>0):
                    nomask = nomask * (torch.max(p_g_iou).item()<self.ignore_thresh)
                noobjmask[bs, ik, gtxy[:, 0], gtxy[:, 1]] = nomask
                tsl = torch.ones(target_scale[bs, ik, gtxy[:,1], gtxy[:,0]].size())
                tsl[tmpmask] = boxloss_scale
                target_scale[bs, ik, gtxy[:,1], gtxy[:,0]] = tsl

            # targetmask[bs, best_n, gtxy[:,1], gtxy[:,0], :] = 1 #best_n的地方计算objectness
            # tx[bs, best_n, gtxy[:,1], gtxy[:,0]] = batch_gt[:, 1] * scale
            # ty[bs, best_n, gtxy[:,1], gtxy[:,0]]  = batch_gt[:, 2] * scale
            # tw[bs, best_n, gtxy[:,1], gtxy[:,0]]  = batch_gt[:, 3] * scale #torch.log(batch_gt[:, 3]/(mask_anchorw[best_n]+1e-10))
            # th[bs, best_n, gtxy[:,1], gtxy[:,0]]  = batch_gt[:, 4] * scale #torch.log(batch_gt[:, 4]/(mask_anchorh[best_n]+1e-10))
            ...
            # # print(self.mask)
            # # ioumask = ioumask[:, self.mask]
            # # print(iouanchor, batch_gtxyxy.shape, anchorxyxy.shape)
            # # print(ioumask)
            # # print(iousum)
            # # print(whe)
            # # exit(0)
            ...
            #     tw[bs, best_n, gtxy[:,1], gtxy[:,0]]  = torch.log(batch_gt[:, 3]/(mask_anchorw[best_n]+1e-10))
            #     th[bs, best_n, gtxy[:,1], gtxy[:,0]]  = torch.log(batch_gt[:, 4]/(mask_anchorh[best_n]+1e-10))
            #     classes[bs, best_n, gtxy[:,1], gtxy[:,0], batch_gt[:, 0].long()] = 1
        ...
        # lossx = self.BCE_scale(cxp*objmask, tx*objmask)
        # lossy = self.BCE_scale(cyp*objmask, ty*objmask)
        # lossw = self.MSE(wp*objmask, tw*objmask)
        # lossh = self.MSE(hp*objmask, th*objmask)
        clamask = torch.unsqueeze(objmask, dim = 4)
        lossobj = self.BCE(confp*objmask, gtconf*objmask)
        lossclasses = self.BCE(classesp*clamask, classes*clamask)

        #计算相应的loss损失
        # showiou = lossiou/batch_size        
        lossiou = torch.sum(torch.FloatTensor(glossiou))/batch_size
        ...
        # print(lossiou, lossobj, lossclasses)
        loss = lossiou + lossobj + lossclasses
        ...
        if torch.sum(objmask).item()==0:
            ...
            objectness = torch.mean(confp[objmask]).item()
        noobjectness = torch.mean(confp[noobjmask]).item()
        return loss, objectness, recall50.item(), recall75.item(), noobjectness, lossiou
...
class Yolov3(nn.Module):
    ...
    def forward(self, x, gt=''):
        ...
        if len(self.gt)!=0:
            ...
            return result3, result2, result1, objectness, recall50, recall75, noobjectness
        else:
            #检测或者测试时返回值
