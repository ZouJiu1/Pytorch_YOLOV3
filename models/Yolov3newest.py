...
class header_block(nn.Module):
    def __init__(self, num_classes, beginchannel, channel, upornot=True):
        ...
        self.conv2 = nn.Conv2d(channel, beginchannel, 3, stride=1, padding=1)
        self.norm2 = nn.BatchNorm2d(beginchannel)
        self.leaky_relu = nn.LeakyReLU(negative_slope=0.1)
        self.conv3 = nn.Conv2d(beginchannel, channel, 1, stride=1)
        self.norm3 = nn.BatchNorm2d(channel)
        self.conv4 = nn.Conv2d(channel, beginchannel, 3, stride=1, padding=1)
        self.norm4 = nn.BatchNorm2d(beginchannel)
        self.conv5 = nn.Conv2d(beginchannel, channel, 1, stride=1)
        self.norm5 = nn.BatchNorm2d(channel)

        #yolo1,接conv5
        self.conv7 = nn.Conv2d(channel, beginchannel, 3, stride=1, padding=1)
        self.norm7 = nn.BatchNorm2d(beginchannel)
        self.conv8 = nn.Conv2d(beginchannel, (5+num_classes)*3, 1, stride=1)
        self.relu = nn.ReLU()

        #upsample,接conv5
        self.conv9 = nn.Conv2d(channel, channel//2, 1, stride=1)
...
class yolo(nn.Module):
    ...
    def forward(self, prediction, gt):
        ...
        objmask = torch.BoolTensor(batch_size, num_anchors, width, height).fill_(False).to(self.device)
        noobjconfmask = torch.BoolTensor(batch_size, num_anchors, width, height).fill_(True).to(self.device)
        ...
        # print("cor: ", num_correct, all_gtnums, nProposals, recall, precision)
        ...
        conf_mask_false = Variable(conf_mask_false.type(torch.BoolTensor), requires_grad=False).to(self.device)
        ...
        # lossx         =  self.BCE_scale(cxp[objmask], offsetctx[objmask])
        # lossy         =  self.BCE_scale(cyp[objmask], offsetcty[objmask])
        # print(wp[objmask], offsetw[objmask], hp[objmask], offseth[objmask], smallscales[objmask])
        lossw         =  self.MSE(wp[objmask] * smallscales[objmask], offsetw[objmask] * smallscales[objmask])
        lossh         =  self.MSE(hp[objmask] * smallscales[objmask], offseth[objmask] * smallscales[objmask])
        ...
        lossnoobj   =  self.scale_noobj *  self.BCE(confp[conf_mask_false], objectness[conf_mask_false])
        lossobj     =  self.scale_obj * self.BCE(confp[conf_mask_true], objectness[conf_mask_true])    #self.BCE(confp, objectness)
        ...
        if(torch.sum(objmask).item()==0):
            ...
            classes = torch.mean(classesp[objmask]).item()

        noobjectness = torch.mean(confp[noobjconfmask]).item()
        return loss, objectnessscore, recall50.item(), recall75.item(), noobjectness, lossiou, recall, precision, classes
...
class Yolov3(nn.Module):
    ...
    def forward(self, x, gt=''):
        ...
        result3, objectness3, recall50_3, recall75_3, noobjectness3, \
            lossiou3, recall3, precision3, classes3 = self.yolo3(y3, deepcopy(self.gt))
        ...
        result2, objectness2, recall50_2, recall75_2, noobjectness2, \
            lossiou2, recall2, precision2, classes2 = self.yolo2(y2, deepcopy(self.gt))
        ...
        result1, objectness1, recall50_1, recall75_1, noobjectness1, \
            lossiou1, recall1, precision1, classes1 = self.yolo1(y1, deepcopy(self.gt))
        ...
        if len(self.gt)!=0:
            ...
            recall50 = (recall50_1 + recall50_2 + recall50_3)/3
            recall75 = (recall75_1 + recall75_2 + recall75_3)/3
