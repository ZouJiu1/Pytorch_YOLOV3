...
class yolo(nn.Module):
    ...
    def forward(self, prediction, gt):
        ...
        for bs in range(batch_size):
            ...
            for i in range(height):
                for j in range(width):
                    ...
                    max_val, max_ind  = torch.max(g_p_iou, dim = 0)
                    for mv in range(len(max_val)):
                        if max_val[mv] > self.ignore_thresh:
                            noobjconfmask[bs, mv, i, j] = 0
            ...
            for ind, rectangle in enumerate(batch_gt):
                ...
                if(g_a_maxind.item() in self.mask): #属于该层的yolo_layer predict
                    ...
                    print()
                    score = confp[bs, a_ind, i, j][0].item()
        ...
        lossx         =  self.MSE(cxp[objmask] * boxloss_scale[objmask], offsetctx[objmask] * boxloss_scale[objmask])
        lossy         =  self.MSE(cyp[objmask] * boxloss_scale[objmask], offsetcty[objmask] * boxloss_scale[objmask])
        lossw         =  self.MSE(wp[objmask] * boxloss_scale[objmask], offsetw[objmask] * boxloss_scale[objmask])
        lossh         =  self.MSE(hp[objmask] * boxloss_scale[objmask], offseth[objmask] * boxloss_scale[objmask])

        self.scale_noobj = 1
        self.scale_obj   = 1
        lossnoobj   =  self.scale_noobj *  self.MSE(confp[conf_mask_false], objectness[conf_mask_false])
        lossobj     =  self.scale_obj * self.MSE(confp[conf_mask_true], objectness[conf_mask_true])    #self.BCE(confp, objectness)

        lossclasses   =  self.BCE(classesp[objmask], classes[objmask]) * (1/batch_size)
