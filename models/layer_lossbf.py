...
def calculate_losses_darknetRevise(prediction, labels, model, ignore_thresh, \
    ...
    for i in range(batchsize):
        ...
        for ia in range(len(arg)):
            ...
            col_choose = torch.concat([argind + (len(anchors) * i), col_choose], dim = 0)
            maxind     = torch.concat([torch.ones((len(argind)), device = model.device, dtype=torch.long) * ia + pre, maxind], dim = 0)
            confi[argind] = True
...
##############-------------------------------------###############################################
    ...
    if len(model.yolo)==2:
        ...
        strides[(~mk)&mk0] = 16
        strides[~mk0] = 32
    del mask, Tindexs_all, col_choose, maxind, delta_mask, deltak, confi_masks, max_ind, anchors, tek
...
##############-------------------------------------###############################################
    ...
    del Tindexs_all, Tvalue_all, col_choose, maxind, confi_masks, anchors, tek, predicts
...
def calculate_losses_yolofiveeight(prediction, labels, model, ignore_thresh, iou_thresh, count_scale, \
                                    bce0loss, bce1loss, bce2loss, bcecls, bcecof, mseloss, num_scale = False):
    predicts = []
    anchors = []
    if isinstance(model, (nn.parallel.DistributedDataParallel, nn.parallel.DataParallel)):
...
##############-------------------------------------###############################################
    ...
    # del mask, Tindexs_all, Tvalue_all, col_choose, maxind, delta_mask, deltak, confi_masks, max_ind
    del Tindexs_all, Tvalue_all, col_choose, maxind, confi_masks, anchors, tek, predicts, lastanchor, lasttruth
...
def calculate_losses_20230730(prediction, labels, model, count_scale, ignore_thresh, \
    ...
    for i in range(batchsize):
        ...
        for ia in range(num):
            ...
            if torch.sum(mk)==0:
                mk[:2] = True
            col_choose = torch.concat([col_choose, argind[mk] + (len(anchors) * i)], dim = 0)
