...
def calculate_losses_largearea(prediction, labels, model, count_scale, \
                                bce0loss, bce1loss, bce2loss, bcecls, bcecof, mseloss, gtmask, keypoints, num_scale = False):
    predicts = []
    anchors = []
...
def keypoint_loss(pred_kpts, gt_kpts, kpt_mask, area, device, d=None):
    OKS_SIGMA = np.array([.26, .25, .25, .35, .35, .79, .79, .72, .72, .62, .62, 1.07, 1.07, .87, .87, .89, .89]) / 10.0
    sigmas = torch.from_numpy(OKS_SIGMA).to(device)
    """Calculates keypoint loss factor and Euclidean distance loss for predicted and actual keypoints."""
    if not d:
        d = (pred_kpts[..., 0] - gt_kpts[..., 0]) ** 2 + (pred_kpts[..., 1] - gt_kpts[..., 1]) ** 2
    kpt_loss_factor = (torch.sum(kpt_mask != 0) + torch.sum(kpt_mask == 0)) / (torch.sum(kpt_mask != 0) + 1e-9)
    # return (kpt_loss_factor * (torch.log(d / area.unsqueeze(-1) + 1 + 1e-9) * kpt_mask)).mean()
    # e = d / (2 * (area * self.sigmas) ** 2 + 1e-9)  # from formula
    # kpt_loss_factor*((1 - torch.exp(-d/(s*(4*sigmas**2)+1e-9)))*kpt_mask).mean()
    # kk = (((2 * sigmas.unsqueeze(0)) ** 2) * area.unsqueeze(-1) + 1e-9)
    # e = d / (((2 * sigmas.unsqueeze(0)) ** 2) * area.unsqueeze(-1) + 1e-9)  / 2 # from cocoeval
    e = d / (2 * sigmas.unsqueeze(0)) ** 2 / (area.unsqueeze(-1) + 1e-9)  # from cocoeval
    # e = d / ((2 ** 2) * area.unsqueeze(-1) + 1e-9)  # from cocoeval
    # kkkk = torch.exp(-e)
    # kkk = (1 - torch.exp(-e)) * kpt_mask
...
def calculate_losses_yolofive(prediction, labels, model, ignore_thresh, iou_thresh, count_scale, \
    ...
    pre_num = 0
    total_pixel = 0
    stridek = torch.tensor([], dtype = torch.float32).to(model.device)
    for i in range(len(model.yolo)):
        num = prediction[i][0].size()[1]
        total_pixel += num
        stridek = torch.concat([stridek, torch.ones(num).to(model.device)*strides[i]], dim = 0)
    length_batchsize = (labels[:, 0] * total_pixel).long()
    length_batchsize = torch.concat([length_batchsize for _ in range(anchor_per_layer)])
    truth_arange = torch.concat([torch.arange(len(labels), device = model.device) for i in range(anchor_per_layer)])
    num_layer = {}
    for i in range(len(model.yolo)):
        anc = prediction[i][1]
        pkeypoint.append(prediction[i][2])
        anchors.append(anc)
        num_layer[i] = len(anc)
        predicts.append(prediction[i][0][:, :, :(1 + 2 + 2)])
        masks.append(prediction[i][0][:, :, (1 + 2 + 2):])
        mapstride = mapsize[i]**2
...
########
    ...
    anchors = torch.concat(anchors, dim=(0)).float()
    predicts = torch.concat(predicts, dim=(1))
    pkeypoint = torch.concat(pkeypoint, dim=(1))
    masks = torch.concat(masks, dim=(1))
    n, _, kl = predicts.size()
    predicts = torch.reshape(predicts, (-1, kl))
    n, _, kl = masks.size()
    masks = torch.reshape(masks, (-1, kl))
    pkeypoint = torch.reshape(pkeypoint, (-1, 16+1, 3))
    ...
    predictind = predictind[ratio]
    truthind = truthind[ratio]
    del label_width, label_height, A_width, A_height, max_width, max_height, ratio, anchorsind
    ...
    kptloss = torch.tensor(0, dtype=torch.float32).to(model.device)
    confi_kpt = torch.tensor(0, dtype=torch.float32).to(model.device)
    ...
    for i in range(batchsize):
        ...
        predict_mask = pre_keepind[ch]
        choose_masks = masks[predict_mask]
        choose_Pkeypoint = pkeypoint[predict_mask]

        onelayer = predict_mask % total_pixel
        # choose_anchor = anchors[onelayer]
        choose_stride = stridek[onelayer]

        # cp = predicts[predict_mask][:, :2*2]
        ...
        choose_kpt = keypoints[truth_index[tch]].float()
        
        choose_kpt[..., 0] = choose_kpt[..., 0] * model.imgsize / choose_stride.unsqueeze(-1)
        choose_kpt[..., 1] = choose_kpt[..., 1] * model.imgsize / choose_stride.unsqueeze(-1)
        kptmask = choose_kpt[..., 2].long()!=0
        kptarea = ((choose_la[:, 2*2] - choose_la[:, 2]) / choose_stride) * ((choose_la[:, 2*2+1] - choose_la[:, 2+1]) / choose_stride)
        # kptarea = ((choose_la[:, 2*2] - choose_la[:, 2]) / model.imgsize) * ((choose_la[:, 2*2+1] - choose_la[:, 2+1]) / model.imgsize)
        # kptarea = (cp[:, 2] / choose_stride) * (cp[:, 3] / choose_stride)
        
        choose_Pkeypoint[:, :, 0] =  choose_Pkeypoint[:, :, 0] / choose_stride.unsqueeze(-1)
        choose_Pkeypoint[:, :, 1] =  choose_Pkeypoint[:, :, 1] / choose_stride.unsqueeze(-1)
        kptloss += keypoint_loss(choose_Pkeypoint, choose_kpt, kptmask, kptarea, model.device)
        confi_kpt += bce2loss(choose_Pkeypoint[..., 2], kptmask.float()).mean()
    ...
    hypbox = 0.06 * 3 / len(model.yolo)  # scale to layers
    # hypcls = 6 * 1 / 80 * 3 / len(model.yolo)  # scale to classes and layers
    hypobj = 1.0 * (model.imgsize / 640) ** 2 * 3 / len(model.yolo)  # scale to image size and layers
    ...
    k = confidence_predict.sigmoid()
    ncof = (torch.sum(k) - torch.sum(k[pre_keepind])) / (len(confidence_predict) - len(pre_keepind))
    # cla = 0   #   torch.mean(classes[torch.arange(len(ll)), ll].sigmoid())
    loss = (confi_kpt + confi_l + iouloss + kptloss + maskloss) * batchsize
    return loss, confi_kpt, confi_l, iouloss, iounow, cof, ncof, maskloss, kptloss, 1   #len(labels)
