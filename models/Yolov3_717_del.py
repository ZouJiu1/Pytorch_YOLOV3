        ...
        for bs in range(batch_size):
            ...
            for ind, rectangle in enumerate(batch_gt):
                ...
                # for i, rectangle in enumerate(batch_gt):
                    center_x = rectangle[1] * width
                    center_y = rectangle[2] * width
                    gtwidth  = rectangle[3] * width
                    gtheight = rectangle[4] * width
                    ...
                    predict_xyxy[:, 0] = ((predict_boxes[:, 0] - predict_boxes[:, 2] * 0.5)/width) * self.inputwidth
                    predict_xyxy[:, 1] = ((predict_boxes[:, 1] - predict_boxes[:, 3] * 0.5)/width) * self.inputwidth
                    predict_xyxy[:, 2] = ((predict_boxes[:, 0] + predict_boxes[:, 2] * 0.5)/width) * self.inputwidth
                    predict_xyxy[:, 3] = ((predict_boxes[:, 1] + predict_boxes[:, 3] * 0.5)/width) * self.inputwidth
