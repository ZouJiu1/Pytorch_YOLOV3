...
def iou_p_g(groundtruth, predict):
    ...
    join = (torch.min(xmax, xmaxp) - torch.max(xmin, xminp)).clamp(0)*\
        (torch.min(ymax, ymaxp) - torch.max(ymin, yminp)).clamp(0)
...
class yolo(nn.Module):
    ...
    def forward(self, prediction, gt):
        ...
        classes = self.FloatTensor(batch_size, num_anchors, width, height).fill_(0)
        ...
        for bs in range(batch_size):
            ...
            classes[bs, best_n, gtxy[:,1], gtxy[:,0]] = batch_gt[:, 0]
        ...
        #计算相应的loss损失
        lossx = self.MSE(cxp[objmask], tx[objmask])
        ...
        lossnoobjectness = self.BCE(confp[noobjmask], gtconf[noobjmask])
        ...
        if torch.sum(objmask).item()==0:
            ...
            loss = 2.5*(lossx + lossy + lossw + lossh) + 1*lossobjectness + 0.5*lossnoobjectness + lossclasses + 2.5*lossiou # 5*lossobjectness
