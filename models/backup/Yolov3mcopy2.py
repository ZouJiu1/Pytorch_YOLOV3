...
class yolo(nn.Module):
    ...
    def forward(self, prediction, gt):
        ...
        mask_anchor_w = anchor_w[:,self.mask,:,:]
        mask_anchor_h = anchor_h[:,self.mask,:,:]
        ...
        targetmask = self.BoolTensor(batch_size, num_anchors, width, height, 4+self.num_classes).fill_(0)
        ...
        for bs in range(batch_size):
            ...
            # objmask[bs, best_n, gtxy[:,1], gtxy[:,0]] = 1 #best_n的地方计算objectness
            # noobjmask[bs, best_n, gtxy[:,1], gtxy[:,0]] = 0 #不计算 noobjectness

            # for i, gtiou in enumerate(iouresult):   #大于给定阈值的地方不计算 not objectness
            #     # noobjmask[bs, gtiou>=self.ignore_thresh, gtxy[:,1][i], gtxy[:,0][i]] = 0
            #     # print(gtiou>=self.ignore_thresh,self.ignore_thresh)
            #     if torch.sum(gtiou>=self.ignore_thresh).item()!=0:
            #         indexthre = torch.argmax(gtiou)
            #         # print(bs, gtiou[indexthre])
            #         objmask[bs, indexthre, gtxy[:,1][i], gtxy[:,0][i]] = 1
            #         noobjmask[bs, indexthre, gtxy[:,1][i], gtxy[:,0][i]] = 0
            #         # objmask[bs, gtiou>=self.ignore_thresh, gtxy[:,1][i], gtxy[:,0][i]] = 1
            #         # print(torch.argmax(gtiou), torch.sum(gtiou>=self.ignore_thresh).item(), objmask[bs, :, gtxy[:,1][i], gtxy[:,0][i]])
            ...
            tw[bs, best_n, gtxy[:,1], gtxy[:,0]]  = torch.log(batch_gt[:, 3]/(anchorw[best_n]+1e-7))
            th[bs, best_n, gtxy[:,1], gtxy[:,0]]  = torch.log(batch_gt[:, 4]/(anchorh[best_n]+1e-7))
        ...
        # gtconf = targetmask.float()

        lossobj = self.BCE(confp*objmask, targetmask*objmask)
        lossclasses = self.BCE(classesp*targetmask[:,:,:,:,4:], classes*targetmask[:,:,:,:,4:])

        #计算相应的loss损失
        lossx = self.MSE(cxp, tx[objmask])
        lossy = self.MSE(cyp[objmask], ty[objmask])
        lossw = self.MSE(wp[objmask], tw[objmask])
        lossh = self.MSE(hp[objmask], th[objmask])
        lossobjectness = self.BCE(confp[objmask], gtconf[objmask])
        lossnoobjectness = self.BCE(confp[~objmask], gtconf[~objmask])
        # print(confp[noobjmask], gtconf[noobjmask])
        lossclasses = self.CE(classesp[objmask], classes[objmask].long())
        ...
        recall50 = torch.sum(iou_score>0.5)/torch.sum(objmask)
        recall75 = torch.sum(iou_score>0.75)/torch.sum(objmask)

        # print((lossx + lossy + lossw + lossh).item(), lossobjectness.item(), lossnoobjectness.item(), lossclasses.item(), lossiou.item())
        if torch.sum(objmask).item()==0:
            loss = lossnoobjectness
            return loss, 0, 0, 0, torch.mean(confp[noobjmask]).item(), 0
        else:
            loss = 2.5*(lossx + lossy + lossw + lossh) + 5*lossobjectness + 0.5*lossnoobjectness + lossclasses + 2.5*lossiou # 5*lossobjectness
        ...
        return loss, torch.mean(confp[objmask]).item(), recall50.item(), recall75.item(), torch.mean(confp[noobjmask]).item(), showiou
    ...
    def forward(self, x, gt=''):
        ...
        if len(self.gt)!=0:
            ...
            for predx, predy, predw, predh, confp, classesp in results:
                ...
                # mask = confp>self.score_thresh    ##[2, 3, 13, 13]
                for bs in range(batch_size):
                    mas = mask[bs, :, :, :]
                    mas = torch.unsqueeze(mas, 0)
                    # print('mas: ', mas.size())
                    predx = torch.unsqueeze(predx[mas], 1)/feature_scale
                    predy = torch.unsqueeze(predy[mas], 1)/feature_scale
                    predw = torch.unsqueeze(predw[mas], 1)/feature_scale
                    predh = torch.unsqueeze(predh[mas], 1)/feature_scale
                    scoresmaxp = torch.unsqueeze(scoresmaxp[mas], 1) 
                    scores_labelp = torch.unsqueeze(scores_labelp[mas], 1)
                    #[cx, cy, w, h, maxscore, label]
                    predict = list(torch.cat([predx, predy, predw, predh, scoresmaxp, scores_labelp], dim=1).detach().cpu().numpy())
                    # print('befor nms: ',predict.shape, 'nms threshold: ', self.nms_thresh, 'score threshold: ', self.score_thresh)
