...
from models.layer_yolo import yololayer_seg
...
class yolovKKNet_backbone(nn.Module):
    def __init__(self, num_classes):
        ...
        self.out0 = ConvBlock_LN(128, (5 + num_classes + 32)*3, 1, 1, 0, act = False)
        self.out1 = ConvBlock_LN(256, (5 + num_classes + 32)*3, 1, 1, 0, act = False)
        self.out2 = ConvBlock_LN(512, (5 + num_classes + 32)*3, 1, 1, 0, act = False)
    ...
    def forward(self, x):
        ...
        y1_out = self.out1(y1)
        y0_out = self.out0(y0)
        return y0_out, y1_out, y2_out, prototype  ##  small obj       middle obj         big obj,       prototype
...
class YolovKKNet(nn.Module):
    def __init__(self, num_classes, anchors, device, imgsize):
        ...
        self.yolo = [yololayer_seg(self.device, len(self.anchors[0]), self.num_classes) for _ in range(len(anchors))]
    ...
    def forward(self, x, yolovfive = False):
        ...
        if self.training:
            ...
            return (torch.cat(kee, 1), prediction[-1])
