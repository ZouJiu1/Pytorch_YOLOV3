...
class yolo(nn.Module):
    ...
    def forward(self, prediction, gt):
        ...
        objmask = torch.zeros(batch_size, num_anchors, width, height).to(self.device)
        confmask = torch.ones(batch_size, num_anchors, width, height).to(self.device)
        ...
        for bs in range(batch_size):
            ...
            for i, rectangle in enumerate(batch_gt):
                ...
                if 0:
                    ...
                    iouindmax  = torch.argmax(iouanchor, dim = 0)
                    ioumask = self.BoolTensor(len(iouanchor)).fill_(False)
                    ioumask[iouindmax.item()] = True
                
                confmask[bs, iouanchor > self.ignore_thresh, int(center_x), int(center_y)] = 0
                ...
                objmask[bs, ioumask, int(center_x), int(center_y)] = 1
                confmask[bs, ioumask, int(center_x), int(center_y)] = 0
        ...
        conf_mask_false = confmask - objmask
        ...
        self.BCE_scale = nn.BCELoss().to(self.device)
