...
class trainDataset(Dataset):
    def __init__(self, traintxt, stride, anchors, anchor_per_layer, \
                device, inputwidth, numclasses, aug=True,\
    ...
    def __getitem__(self, idx):
        ...
        # print(bboxes, labels, image.shape)
        if self.aug:
            ...
            for ind, i in enumerate(labels):
                gt.append([labels[ind], bboxes[ind][0], bboxes[ind][1], bboxes[ind][2], bboxes[ind][3], 0])
            image = Image.fromarray(image)

        if self.transform:
            image = self.transform(image)
        
        label_sbbox, label_mbbox, sbbox, mbbox = self.__create_labels(gt)
        ...
        return image, label_sbbox, label_mbbox, sbbox, mbbox
    ...
    def __create_labels(self, bboxes):
        ...
        labels = [ np.zeros((int(train_outsize[i]), int(train_outsize[i]), \
            self.anchor_per_layer, 5 + self.numclasses)) for i in range(2)]
        
        max_rec= self.maxrec #提前配置的单个图片最多框的个数
        bboxes_txywh = [np.zeros((max_rec, 4)) for _ in range(2)]
        bboxes_count = np.zeros((2,))
        ...
        for i in range(len(bboxes)):
            ...
            for ja in range(2):              #small boxes、middle boxes、large boxes
        ...
        label_sbbox, label_mbbox = labels
        sbbox, mbbox = bboxes_txywh
        return label_sbbox, label_mbbox, sbbox, mbbox
